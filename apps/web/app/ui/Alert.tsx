...
export function Alert({ variant, title, children, className }: AlertProps) {
  const { icon: Icon, containerClass, iconClass, titleClass, textClass } = variantConfig[variant]

  return (
    <div className={cx('flex w-full rounded-lg px-3 py-2 text-sm', containerClass, className)}>
      <Icon
      ...
      <div className={cx('flex grow flex-col gap-y-1', textClass)}>
        {title && <span className={cx('font-semibold', titleClass)}>{title}</span>}
        <div>{children}</div>
      </div>
    </div>
  )
